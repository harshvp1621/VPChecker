...
def main():
    file_list = set()
    for root, dirs, files in os.walk("../docker_vols", topdown=False):
        for file_name in files:
            full_file_path = f"{root}/{file_name}"
            if full_file_path.endswith(".fcg.json"):
                full_bin_path = full_file_path.split(".fcg.json")[0]
                file_list.add(full_bin_path)
            elif full_file_path.endswith(".tar.gz"):
                full_lib_path = full_file_path.split(".tar.gz")[0]
                file_list.add(full_lib_path)
