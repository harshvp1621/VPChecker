...
# Shared Libraries are all dumped into the libs folder
JSON_FILE_DIR=f"{os.getcwd()}/../../docker_vols/LIBS/libs"
# JSON_FILE_DIR="/esxi2-pool/user/Supply_Chain/supply-chain-linux/docker/playg"
...
def create_elf_to_deb_map(curr_deb, elf_dep_list):
    ...
    for elf in elf_dep_list:
        if elf not in elf_to_deb_map:
            res_dict[elf] = f"{elf}_neverfound"
            continue
...
def process_tar(tar_file):
    deb_name = tar_file.split("/")[-2]
    elf_name = ""
    elf_csv = ""

    if tar_file.endswith(".tar.gz"):
        # Decompress with pigz
        elf_csv = f"{tar_file.split('.tar')[0]}_{deb_name}_ELF64_elf_info.csv"
        decomp_script = f"{os.getcwd()}/decomp_pigz.sh"
    ...
    try:
        subprocess.run(shlex.split(f"{decomp_script} {tar_file}"))
    except:
        print(str(traceback.format_exc()))
        return 0

    tar_json_file = f"{tar_file}.extracted"
    fcg_json_list = []
    for top_dir, sub_dirs, files in os.walk(f"{tar_json_file}/", topdown=False):
        for file_name in files:
            full_file_path = os.path.join(top_dir, file_name)
            if full_file_path.endswith(".json"):
                fcg_json_list.append(full_file_path)
    LIB_ERROR_FILE = f"{tar_file}.error"
    global db
    for fcg_json in fcg_json_list:
        try:
            process_fcg(fcg_json, elf_name, sanitize_deb_name(deb_name))
        except:
            with open(LIB_ERROR_FILE, "a+") as error_file:
                error_file.write(f"======= Error in {fcg_json} ========")
                error_file.write(str(traceback.format_exc()))
            continue

    # Cleanup the extracted directory
    subprocess.run(shlex.split(f"rm -rf {tar_file}.extracted"))
    subprocess.run("sync")
    with open(f"{tar_file}.tardone", 'a+') as done_file:
...
def process_fcg(fcg_json_file, elf_name, deb_name):
    ...
    # Before opening the JSON file, check if the function was processed previously
    exported_func_name = fcg_json_file.split(".json")[0].split("out_")[1]
    ...
    # if func_vertex_collection.has(f"{NODE_COLL_NAME}/{exported_func_name}@{elf_name}@{deb_name}"):
    if check_doc_present(NODE_COLL_NAME, f"{NODE_COLL_NAME}/{exported_func_name}@{elf_name}@{deb_name}"):
        # This function was already processed as an entrypoint, so we return
        if len(edge_collection_dir.edges(f"{NODE_COLL_NAME}/{exported_func_name}@{elf_name}@{deb_name}")):
            return

    try:
        with open(fcg_json_file, "r") as f:
            lines = f.readlines()
            sysfilter_analysis = json.loads(lines[0].strip())
...
def main():
    ...
    # process_fcg("JSONS/vim.basic.fcg.json")
    tar_file_size_dict = {}
    ...
    for top_dir, sub_dirs, files in os.walk(f"{JSON_FILE_DIR}/", topdown=False):
        for file_name in files:
            ...
            if os.path.islink(full_file_path):
                continue
            if full_file_path.endswith(".tar.gz") or full_file_path.endswith(".tar.xz"):
                if os.path.exists(f"{full_file_path}.tardone"):
                    continue
                else:
                    tar_file_size_dict[full_file_path] = os.path.getsize(full_file_path)

    tar_file_size_dict_sorted = dict(sorted(tar_file_size_dict.items(), key=lambda x:x[1]))
    tar_file_list = [ t for t in tar_file_size_dict_sorted.keys() ]
    # for t in tar_file_list:
    #    print(t)
    pool = multiprocessing.Pool(initializer=get_arango_connection, processes=os.cpu_count())

    pool.map(process_tar, tar_file_list)
